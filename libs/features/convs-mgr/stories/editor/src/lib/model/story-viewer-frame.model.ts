...
export class StoryViewerFrame {
  ...
  private _frameChanges$$: BehaviorSubject<StoryViewerState> =
    new BehaviorSubject({ loading: true } as any as StoryViewerState);
   ...
   */
  async init(state: StoryViewerState) {
    this._state = state;
  ...
  drawConnections() {
    ...
    for (const conn of this._connections) {
      ...
      // more infor on connect can be found ->
      // https://docs.jsplumbtoolkit.com/community-2.x/current/articles/connections.html
  ...
  newBlock(type: StoryBlockTypes, coordinates?: Coordinate) {
    ...
    const filteredBlocks = this._blocks.filter(
      (block) => block.id !== 'story-end-anchor'
    );
